...
const PricingSection = () => {
  ...
  const getCardStyle = (plan: Plan) => {
    ...
    return {
      ...
      button: isHighlighted || plan.key === "yearly" ? "bg-primary text-white" : "bg-title text-white",
      ...
      <div className="max-w-[1120px] mx-auto">
        <div className="flex flex-col items-center justify-center text-center text-muted mb-2">
          ...
          <div className="relative inline-flex items-center rounded-full p-1 mb-8">
            <span className={`px-3 text-sm sm:text-base text-title ${!isMonthly ? "" : "opacity-40"}`}>
            ...
            >
              <div className={`w-5 h-5 bg-title rounded-full absolute transition-all duration-200 ${
              ...
              <div key={plan.key} className="relative mt-12 first:mt-0 md:mt-0">
                {plan.tags.length > 0 && (
                  <div className={`absolute -top-2 left-0 right-0 flex justify-center gap-2 ${
                    ...
                    (!isMonthly && plan.key === "twoYears") ? "scale-110 !-top-10 z-50" : ""
                  }`}>
                    {plan.tags.map((tag) => (
                      <div
                        key={tag.key}
                ...
                <Card className={`overflow-hidden ${style.card}`}>
                  <div className="px-14 py-12 sm:px-10">
                    ...
                    {plan.features.map((feature) => (
                        <li key={feature.key} className="flex items-start sm:items-center gap-3">
                          <Image
